...
categories: ["封档"]
description: "封档"
publishDate: 2023-03-02 21:10:10
---

# Spring Data Redis

> **Spring Data Redis 是Spring框架提供的用于操作Redis的方式，本文涵盖了Redis的安装、Spring Cache结合Redis的使用、Redis连接池的使用和RedisTemplate的使用等内容**

## Redis安装

### Windows

> **本文使用的Redis版本为5.0.14.1**

- **下载Windows版本的Redis，下载地址：https://github.com/tporadowski/redis**
...
> **当SpringBoot结合Redis来作为缓存使用时，最简单的方式就是使用Spring Cache，使用它我们无需知道Spring中Redis的各种操作，仅仅通过它提供的@Cacheable、@CachePut、@CacheEvict和@EnableCaching等注解就可以实现缓存功能**
...
**使用该注解的方式执行时会清空指定的缓存，`一般使用在更新或删除方法上`，可以设置如下属性：**
...
- **修改配置文件`application.yml`，添加Redis的连接配置**
...
public class MallTestRedisApplication
{

    public static void main(String[] args)
    {
        SpringApplication.run(MallTestRedisApplication.class, args);
    }
}
```
...
![img](https://www.macrozheng.com/assets/spring_boot_redis_04.002a198b.png)

### 存储JSON格式数据

> **此时我们就会想到有没有什么办法让Redis中存储的数据变成标准的JSON格式，然后可以设置一定的过期时间，不设置过期时间容易产生很多不必要的缓存数据**

- **我们可以通过给`RedisTemplate`设置JSON格式的序列化器，并通过配置`RedisCacheConfiguration`设置超时时间来实现以上需求，此时别忘了去除启动类上的`@EnableCaching`注解，具体配置类`RedisConfig`代码如下**
...
- **此时我们再次调用获取商品详情的接口进行测试，会发现Redis中已经缓存了标准的JSON格式数据，并且超时时间被设置为1天**
...
- **修改`application.yml`添加Lettuce连接池配置，用于配置线程数量和阻塞等待时间**
...
- **由于SpringBoot 2.x中默认并没有使用Redis连接池，所以需要在`pom.xml`中添加`commons-pool2`的依赖**
...
> **定义Redis操作业务类，在Redis中有几种数据结构，如普通结构（对象）、Hash结构、Set结构、List结构，该接口中定义了大多数常用操作方法**
...
> **测试RedisService中缓存操作的Controller**
